"""Module exposing xslx data parser"""
import pandas as pd
...
_AUCTION_MAP = {
    ...
    'seller_name': 'Razão social do vendedor',
    'seller_code': 'CNPJ do\nvendedor',
    'buyer_initials': 'Sigla do\ncomprador\n(3)',
    'buyer_name': 'Razão social do comprador',
    'buyer_code': 'CNPJ do\ncomprador',
    'ceg': 'C.E.G.',
    ...
    'Potência Final\nInstalada C.C.\n(MWp) (235)',
    'energy_physical_guarentee': 'Garantia Física\nda usina\n(MW médio)\n(7)',
...
def parse_xsl(file):
    """Parses xslx auction table from CCEE"""
    _df: pd.DataFrame = pd.read_excel(file,
                                      sheet_name='Resultado Consolidado',
                                      skiprows=9)
    _df = _df.drop(_df.columns[0], axis=1)
    _df.columns = _AUCTION_MAP.keys()
